...
def make_provider(authorize):
    """Helper to quickly build a CredentialsProvider with the LoginSchema."""
    return CredentialsProvider(model=LoginSchema, authorize=authorize)


@pytest.mark.asyncio
async def test_credentials_missing_required_field(user_data):
    """
    ...
    provider = make_provider(authorize=lambda _: user_data_dict)
...
@pytest.mark.asyncio
async def test_credentials_wrong_field_type(user_data):
    """
    ...
    """
    provider = make_provider(authorize=lambda _: user_data())
...
@pytest.mark.asyncio
async def test_credentials_empty_body(user_data):
...
@pytest.mark.asyncio
async def test_credentials_authorize_returns_none():
    """
    ...
    """
    provider = make_provider(authorize=lambda _: None)
...
@pytest.mark.asyncio
async def test_credentials_authorize_returns_false():
    """Same as None — returning False should also be treated as auth failure."""
    provider = make_provider(authorize=lambda _: False)
    result = await provider.authenticate({"email": "user@example.com", "password": "wrong"})
...
@pytest.mark.asyncio
async def test_credentials_authorize_returns_non_dict():
    """
    ...
    """
    provider = make_provider(authorize=lambda _: "user@example.com")
...
@pytest.mark.asyncio
async def test_credentials_authorize_raises_exception():
    """
    ...
    provider = make_provider(authorize=crashing_authorize)
    result = await provider.authenticate({"email": "user@example.com", "password": "pass"})
...
@pytest.mark.asyncio
async def test_credentials_sync_authorize_success(user_data):
    """
    ...
    provider = make_provider(authorize=sync_authorize)
...
@pytest.mark.asyncio
async def test_credentials_async_authorize_success(user_data):
    """
    ...
    provider = make_provider(authorize=async_authorize)
    result = await provider.authenticate({"email": "user@example.com", "password": "correct"})
...
@pytest.mark.asyncio
async def test_credentials_success_returns_user_data(user_data):
    """
    ...
    """
    valid_user = user_data()
    provider = make_provider(authorize=lambda _: valid_user)
    result = await provider.authenticate({"email": "user@example.com", "password": "correct"})
